# CQC COMPLIANCE AI CHAT ASSISTANT — COMPLETE SYSTEM PROMPT & IMPLEMENTATION GUIDE

> **For Cursor:** This file contains TWO things:
> 1. **Implementation instructions** (Section A) — How to build the chat using Vercel AI SDK + Anthropic
> 2. **The complete system prompt & knowledge base** (Section B) — The actual content that powers the AI assistant
>
> Build Section A first, then paste Section B as the system prompt.

---

# SECTION A: IMPLEMENTATION WITH VERCEL AI SDK
...
The `ai` package is the Vercel AI SDK. `@ai-sdk/anthropic` is the Anthropic provider for it. This gives us streaming out of the box.
...
export async function POST(req: Request) {
  ...
  if (user?.organization_id) {
    ...
    const { data: scores } = await supabase
      ...
      .select('overall_score, predicted_rating, safe_score, effective_score, caring_score, responsive_score, well_led_score, calculated_at')
...
- Current Compliance Score: ${scores?.overall_score ?? 'Not yet calculated'}%
- Predicted CQC Rating: ${scores?.predicted_rating || 'Not yet calculated'}
- Domain Scores: Safe ${scores?.safe_score ?? '?'}%, Effective ${scores?.effective_score ?? '?'}%, Caring ${scores?.caring_score ?? '?'}%, Responsive ${scores?.responsive_score ?? '?'}%, Well-Led ${scores?.well_led_score ?? '?'}%
- Last Data Sync: ${scores?.calculated_at || 'Never'}
...
## Frontend Chat Component: `components/ai-chat/chat-panel.tsx`
...
export function AIChatPanel() {
  ...
  return (
    <>
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-[420px] h-[600px] bg-background border border-border rounded-xl shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b bg-muted/50">
            <div className="flex items-center gap-2">
              <Bot className="h-5 w-5 text-primary" />
              <div>
                <h3 className="font-semibold text-sm">CQC Compliance Assistant</h3>
                <p className="text-xs text-muted-foreground">Ask anything about CQC compliance</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-muted-foreground text-sm py-8">
                ...
                <p className="mt-1 text-xs">Ask me about CQC compliance, your gaps, policies, or how to use this tool.</p>
...
## Add to Layout: `app/(dashboard)/layout.tsx`

```typescript
import { AIChatPanel } from '@/components/ai-chat/chat-panel';

// Inside the layout's return, add before the closing tag:
<AIChatPanel />
```

## System Prompt File: `lib/ai/chat-system-prompt.ts`

```typescript
export const SYSTEM_PROMPT = `
// PASTE THE ENTIRE SECTION B BELOW HERE
`;
```

---

# SECTION B: THE COMPLETE SYSTEM PROMPT & KNOWLEDGE BASE

Everything below this line is the system prompt content. Copy it into `lib/ai/chat-system-prompt.ts`.

---

You are the CQC Compliance Assistant built into the Consentz CQC Compliance Module. You are an expert in CQC regulation, healthcare compliance, and the Consentz platform. You help aesthetic clinic owners, care home managers, compliance officers, and their staff understand CQC requirements, navigate the compliance tool, improve their compliance posture, and prepare for inspections.
